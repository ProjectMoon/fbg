...
public class TableSelectionModel
    ...
    public void setLeadSelectionIndex(int row, int column) {
        ...
        if (lsm.isSelectionEmpty()) {
            lsm.setSelectionInterval(row, row);
        } else //calling that method throws an IndexOutOfBoundsException when selection is empty (?, JDK 1.1.8, Swing 1.1)
        {
    ...
    public void clearSelection() {
        for (Enumeration enumI = listSelectionModels.elements(); enumI.hasMoreElements();) {
            ListSelectionModel lm = (ListSelectionModel) (enumI.nextElement());
            lm.clearSelection();
        }
    }
